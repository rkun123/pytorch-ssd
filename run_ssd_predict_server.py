...
def predict(byte_data, width, height):

    image_arr = np.frombuffer(byte_data, dtype=np.uint8)
    image = np.array(image_arr).reshape((height, width, 3))
    ...
    for i in range(boxes.size(0)):
        ...
        cv2.putText(orig_image, label,
                    ...
                    (255, 0, 255),
                    2)  # line type
    path = "run_ssd_example_output_transfered.jpg"
    t = MeasureTime('cv2.imwrite')
    cv2.imwrite(path, orig_image)
    # cv2.imwrite('./transfered_image.jpg', image)
    t.stop()
    print(f"Found {len(probs)} objects. The output image is {path}")


def extruct_image_shape(data: bytes):
    width = data[0:2]
    height = data[2:4]
    if len(data) < 4:
        return None, None
...
while True:
    recvd_data = client.recv(buffer_size)
    # print("[*] Received Data : {}".format(len(recvd_data)))
    print(recvd_data[-1] == 0)
    ...
    if recvd_data[-1] == 0:
        data = data[:-1]
        break
    print("[*] Received Data : {} / {}".format(len(data), len(recvd_data)))

print("Received All datas for {}".format(len(data)))
client.send(b"OK")

width, height = extruct_image_shape(data)

print("shape: {}x{}".format(width, height))

predict(data[4:], width, height)
